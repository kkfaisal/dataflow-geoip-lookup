"""
Redading data from PubSub topic and find location based on ip present in input data and write it to bigquery table
Dataflow used in stream mode mode.

Pubsub Message format : {'event_id': STRING, 'ip': STRING, 'date_time':TIMESTAMP}
...
def parse_pubsub(line):
    ...
    return (record['event_id']), (record['ip']), (record['date_time'])
...
def run(projectId, src_dataset, src_tablename, dest_dataset, dest_tablename, gcs_location_prefix, jobname):
    ...
    worker_options.max_num_workers = 500
    ...
    with beam.Pipeline(options=options) as p:
        lines = (p | 'ReadSubSub' >> beam.io.ReadStringsFromPubSub('projects/mm-bq-eval/topics/df_stream')
                 | 'ParsePubSub' >> beam.Map(parse_pubsub)
                 | 'Create table rows' >> beam.Map(
            lambda (evid_bq, ip_bq, datetime_bq): {'event_id': evid_bq, 'ip': ip_bq, 'date_time': datetime_bq})
                 | 'Enrich GeoIP' >> beam.ParDo(GeoIpFn()))

        lines | 'WriteBQ' >> beam.io.Write(
            beam.io.BigQuerySink(
                table=dest_tablename,
                dataset=dest_dataset,
                schema='event_id:STRING, ip:STRING, date_time:TIMESTAMP',
